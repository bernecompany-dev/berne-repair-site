...
export default async function ServiceCityPageES({ params }: Props) {
  ...
  const comboFaqs = [
    {
      question: `¿Qué tan rápido pueden llegar a ${city.name} para un ${service.seoNoun}?`,
    ...
    {
      question: `¿Cuánto cuesta la reparación de ${service.shortName.toLowerCase()} en ${city.name}?`,
      answer: `Nuestra visita técnica fija de $${COMPANY.serviceCallPrice} lleva al técnico a su dirección en ${city.name} e incluye un diagnóstico completo. Si aprueba la reparación, esos $${COMPANY.serviceCallPrice} se aplican al total. La mayoría de reparaciones de ${service.shortName.toLowerCase()} cuestan entre $150 y $600 dependiendo de la pieza.`,
    },
    {
    ...
    <>
      <section className="relative isolate overflow-hidden border-b border-border/60">
        ...
        <div className="container-prose pt-14 pb-16 sm:pt-20 sm:pb-20">
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-xs text-muted-foreground" aria-label="Breadcrumb">
            ...
            <span aria-hidden>/</span>
            <Link href={`/es/services/${service.slug}`} className="hover:text-foreground">{service.name}</Link>
            <span aria-hidden>/</span>
          ...
          <p className="mt-6 max-w-3xl text-lg leading-relaxed text-muted-foreground sm:text-xl">
            Reparación de {service.seoNoun} el mismo día en {city.name} — sirviendo a {city.neighborhoods.slice(0, 3).join(", ")} y cada barrio del condado de {city.county}. {COMPANY.socialProof.technicians} técnicos con licencia, camiones equipados para {service.brands.slice(0, 3).join(", ")} y todas las marcas principales.
          </p>
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_2fr] lg:items-start">
          <div>
            ...
            <h2 className="heading-section mt-3">
              Lo que nos cuentan los dueños de {service.shortName.toLowerCase()} en {city.name}
            </h2>
      ...
      <section className="border-t border-border/60 bg-background/40">
        <div className="container-prose py-16 grid gap-10 lg:grid-cols-2">
          <div>
            <h3 className="text-lg font-semibold tracking-tight">{service.name} en ciudades cercanas</h3>
            <ul className="mt-5 grid gap-2 sm:grid-cols-2">
              {otherCities.map((c) => (
                <li key={c.slug}>
                  ...
                  >
                    <span className="text-sm font-semibold">{service.shortName} en {c.name}</span>
              ...
              {otherServices.map((s) => (
                <li key={s.slug}>
                  <Link href={`/es/services/${s.slug}/${city.slug}`}
                    className="group flex items-center justify-between rounded-xl border border-border bg-card/40 px-4 py-3 transition-all hover:-translate-y-px hover:border-brand/40 hover:bg-card/60"
                  >
                    <span className="text-sm font-semibold">{s.shortName} en {city.name}</span>
                    <ArrowRight className="size-4 text-muted-foreground transition-all group-hover:translate-x-0.5 group-hover:text-brand" aria-hidden />
                  </Link>
                </li>
              ))}
      ...
      <FAQSection faqs={comboFaqs} title={`${service.name} en ${city.name} — FAQ`} />
      <Contact defaultAppliance={service.slug} defaultCity={city.slug} locale="es" />
      <CTABand locale="es" />

      <JsonLd data={[serviceCityJsonLd(service, city), faqJsonLd(comboFaqs), breadcrumbJsonLd(crumbs)]} />
    </>
  );
}
